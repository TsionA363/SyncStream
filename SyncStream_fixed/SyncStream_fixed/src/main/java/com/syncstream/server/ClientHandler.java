...
public class ClientHandler implements Runnable {
    ...
    private void handleLogin(String[] parts) {
        if (parts.length < 3) {
            send("ERROR:LOGIN format: LOGIN:<user>:<pass>");
            return;
        }
    ...
    private void handleRegister(String[] parts) {
        if (parts.length < 4) {
            send("ERROR:REGISTER format: REGISTER:<user>:<email>:<pass>");
            return;
        }
    ...
    private void handleCreateRoom(String[] parts) {
        ...
        if (parts.length < 3) {
            send("ERROR:CREATE_ROOM format: CREATE_ROOM:<name>:<videoPath>");
            return;
        }
    ...
    private void handleJoinRoom(String[] parts) {
        ...
        if (parts.length < 2) {
            send("ERROR:JOIN_ROOM format: JOIN_ROOM:<roomCode>");
            return;
        }
        ...
        if (room != null) {
            ...
            SyncServer.broadcastExcludeSender(
                    currentRoomId,
                    "USER_JOINED:" + currentUser.getUsername(),
                    this
            );
        } else {
    ...
    private void handleChat(String[] parts) {
        ...
        SyncServer.broadcastToRoom(
                currentRoomId,
                "CHAT:" + currentUser.getUsername() + ":" + message,
                this
        );
    }
    ...
    private void handleReaction(String[] parts) {
        ...
        SyncServer.broadcastToRoom(
                currentRoomId,
                "REACTION:" + currentUser.getUsername() + ":" + emoji,
                this
        );
    }
    ...
    private void handleLeave() {
        if (currentRoomId >= 0) {
            SyncServer.broadcastExcludeSender(
                    currentRoomId,
                    "USER_LEFT:" + getUsername(),
                    this
            );

            endWatchSession();

            DatabaseAPI.leaveRoom(
                    currentRoomId,
                    currentUser.getUserId()
            );
    ...
    private void cleanup() {
        try {
            if (currentRoomId >= 0 && currentUser != null) {
                ...
                SyncServer.leaveRoom(
                        currentRoomId,
                        this
                );

            } else {
                SyncServer.leaveRoom(-1, this);
            }
